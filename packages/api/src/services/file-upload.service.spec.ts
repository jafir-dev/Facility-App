...
describe('FileUploadService', () => {
  ...
  describe('uploadFile', () => {
    ...
    it('should throw error for file too large', async () => {
      ...
      await expect(
        service.uploadFile(largeFile, 'ticket-123', 'user-123', MediaContext.TICKET_PHOTO)
    ...
    it('should throw error for invalid file type', async () => {
      ...
      await expect(
        service.uploadFile(invalidFile, 'ticket-123', 'user-123', MediaContext.TICKET_PHOTO)
      ).rejects.toThrow('File type application/x-tar is not allowed');
    });

    it('should generate secure filename', async () => {
      const dangerousFile = { ...mockFile, originalname: '../../../malicious.jpg' };

      await service.uploadFile(dangerousFile, 'ticket-123', 'user-123', MediaContext.TICKET_PHOTO);
  ...
  describe('uploadMultipleFiles', () => {
    ...
    it('should throw error for too many files', async () => {
      const files = Array(15).fill(mockFile); // 15 files, max is 10

      await expect(
        service.uploadMultipleFiles(files, 'ticket-123', 'user-123', MediaContext.TICKET_PHOTO)
  ...
  describe('deleteFilesForTicket', () => {
    it('should delete all files for a ticket', async () => {
      ...
      expect(fsMock.remove).toHaveBeenCalledTimes(6); // 3 files each for 2 media
    });
  });
  ...
  describe('magic number validation', () => {
    ...
    it('should reject files with spoofed MIME types', () => {
      ...
      expect(validateFileType(exeBuffer, 'image/jpeg')).toBe(false);
    });
  });
});